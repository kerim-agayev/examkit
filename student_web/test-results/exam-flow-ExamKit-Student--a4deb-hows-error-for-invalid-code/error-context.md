...
- Name: exam-flow.spec.ts >> ExamKit Student Flow >> code entry shows error for invalid code
- Location: e2e\exam-flow.spec.ts:42:7
...
Call log:
  ...
  - waiting for locator('button[type=\'submit\']')
    14 × locator resolved to <button disabled type="submit" class="w-full h-[56px] rounded-xl flex items-center justify-center gap-2 text-lg font-semibold bg-text-disabled text-on-primary cursor-not-allowed">Devam →</button>
       - unexpected value "disabled"

```

```yaml
- button "Devam →" [disabled]
```
